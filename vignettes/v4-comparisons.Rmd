...
TC <- data.table::data.table(
  ...
  age.control=c(10, 50))
input.list <- list(
  "nc"=nc::capture_first_melt_multiple(
    TC,
    column=".*?",
    "[.]",
    group=".*"),
  "cdata"=cdata::rowrecs_to_blocks(
    TC,
    controlTable=data.frame(
...
for(pkg in names(input.list)){
  output.list[[pkg]] <- data.table::data.table(input.list[[pkg]])[order(age)]
}
output.list
