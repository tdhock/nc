...
provide a function for melting into multiple output columns. One
example is:

```{r}
iris.parts <- list()
(iris.parts[["nc::capture_first_melt_multiple"]] <- nc::capture_first_melt_multiple(
  iris,
  column=".*?",
  "[.]",
  dim=".*"))
...
The regular expression above specifies that the text before the dot
should be used as the output column name. The regex matches the four
numeric input columns, which are melted into two numeric output
columns. The result is a new data table which can be easily plotted:
...
### Other packages which can melt into several output columns

Now let's see how to do this conversion with some other packages. For
example we could do

```{r}
iris.parts[["cdata::rowrecs_to_blocks"]] <- cdata::rowrecs_to_blocks(
  iris,
...
iris.parts[["stats::reshape"]] <- stats::reshape(
  iris,
  direction="long",
  timevar="dim",
  varying=1:4,
  sep=".")
iris.parts[["tidyr::pivot_longer"]] <- tidyr::pivot_longer(
  iris, 
  cols=1:4, 
...
The same table can be computed in a single step using the different methods above.
...
```{r}
iris.dims <- list()
(iris.dims[["nc::capture_first_melt_multiple"]] <- nc::capture_first_melt_multiple(
  iris,
  part=".*?",
  "[.]",
  column=".*"))
...
```{r}
iris.dims[["cdata::rowrecs_to_blocks"]] <- cdata::rowrecs_to_blocks(
  iris,
  controlTable=data.frame(
    part=c("Petal", "Sepal"),
    ...
    stringsAsFactors=FALSE),
  columnsToCopy="Species")
iris.dims[["tidyr::pivot_longer"]] <- tidyr::pivot_longer(
  iris, 
  cols=1:4, 
...
```{r}
iris.inv <- data.frame(iris)
names(iris.inv) <- sub("(.*?)[.](.*)", "\\2.\\1", names(iris))
iris.dims[["stats::reshape"]] <- stats::reshape(
  iris.inv,
  direction="long",
  timevar="part",
  varying=1:4,
...
the conversions. However the syntax is different:

* cdata requires explicit specification of a control table.
