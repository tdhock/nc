...
- =nc::capture_all_str= only supports capturing multiple matches in a
  ...
  multiple subjects. For multiple subjects, use =nc::capture_all_str=
  inside a =by= block (see [[https://cloud.r-project.org/web/packages/nc/vignettes/v2-capture-all.html][vignette 2]] for more info).
...
stats, data.table, reshape, reshape2, cdata, utils, etc. For a
detailed comparison see [[https://github.com/tdhock/nc-article][my paper about nc]].
