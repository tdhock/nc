...
| pkg::function                   | single | multiple | regex | capture | na.rm | types |
|---------------------------------+--------+----------+-------+---------+-------+-------|
| nc::capture_first_melt_multiple | no     | yes      | yes   | yes     | yes   | any   |
| nc::capture_first_melt          | yes    | no       | yes   | yes     | yes   | any   |
| tidyr::pivot_longer             | yes    | yes      | yes   | yes     | yes   | no    |
| stats::reshape                  | yes    | yes      | yes   | no      | no    | no    |
| data.table::melt                | yes    | yes      | yes   | no      | yes   | no    |
| tidyr::gather                   | yes    | no       | no    | no      | yes   | some  |
| reshape2::melt                  | yes    | no       | no    | no      | yes   | no    |
