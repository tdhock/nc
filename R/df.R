...
### character string, one of
  ...
  for(col.name in names(col.pattern.list)){
    subject.names <- if(.row_names_info(out) > 0L){
      attr(out, "row.names")
    }
    subject.vec <- structure(out[[col.name]], names=subject.names)
...
### like a data.table::data.table then the value is too).
  ...
  df_capture_first(sacct.df, JobID=list(
  ...
  task.pattern <- list(
    ...
    list(
      ...
      range.pattern))
  df_capture_first(sacct.df, JobID=task.pattern)
  ...
  task.type.pattern <- list(
    task.pattern,
    type.pattern, "?")
  df_capture_first(sacct.df, JobID=task.type.pattern)

  ## Match full JobID and Elapsed columns.
  (task.df <- df_capture_first(
    sacct.df,
    JobID=list(
      job=int.pattern,
      task.type.pattern),
